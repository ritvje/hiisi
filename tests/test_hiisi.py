...
class Test(unittest.TestCase):
    ...
    def test_datasets(self):
        self.assertEqual(sorted(list(self.h5file.datasets())), sorted(self.dataset_paths))

    def test_datasets_no_datasets_found(self):
        with hiisi.HiisiHDF('tmp.h5', 'w') as h5f:
            self.assertEqual(list(h5f.datasets()), [])
        os.remove('tmp.h5')

    def test_groups(self):
        self.assertEqual(sorted(list(self.h5file.groups())), sorted(self.group_paths))

    def test_groups_no_groups_found(self):
        with hiisi.HiisiHDF('tmp.h5', 'w') as h5f:
            self.assertEqual(h5f.groups(), ['/'])
        os.remove('tmp.h5')
